...
public class EmailContentBuilder {
	...
	private static final Pattern COMPILED_PATTERN_SRC_URL_DOUBLE = Pattern
			.compile("src=\"([^\"]*)\"", Pattern.CASE_INSENSITIVE);
	...
	private void addTextVersion(Multipart mpRelatedAlternative, String messageText) 
			throws MessagingException {
	...
	private void addHtmlVersion(
		...
		//TODO gestire la codifica inline delle immagini
		//htmlPart.setContent(replaceUrlWithCids(messageHtml, cids), "text/html");
		htmlPart.setContent(messageHtml, "text/html"); // In caso di codifica inline questa riga va sostituita con la precedente
	...
	private String replaceUrlWithCids(
			String html, 
			Pattern pattern,
			String replacement, 
			HashMap<String, String> cids
			) {
		...
		while (matcherCssUrl.find()) {
			...
			matcherCssUrl.appendReplacement(sb,
					replacement.replace("@cid", cid));
		}
