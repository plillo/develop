...
public class ServletContainerBridge extends HttpServlet implements Runnable {
	...
	public void run() {
		if (application.isDirty()) {
			...
			try {
				...
				synchronized (this) {
					if (!isJerseyReady()) {
						System.out.println("Initializing JAX-RS Container.");
						// if jersey has not been initialized - use the init method 
						getServletContainer().init(servletConfig);
						System.out.println("JAX-RS Container initialized.");
					} else {
						// otherwise reload
						isJerseyReady = false;
	...
	private void handleTerminalError(Throwable t) {
		System.out.println("Error during JAX-RS Container initialization. Taking bundle down. ["+ t.toString()+"]");
		try {
			FrameworkUtil.getBundle(this.getClass()).stop();
		} catch (BundleException e) {
			e.printStackTrace();
			System.out.println("error during bundle teardown. ["+ e.toString()+"]");
